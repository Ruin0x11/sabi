...
impl SpriteMap {
    pub fn new<F: Facade>(display: &F) -> Self {
        ...
        let shadow_program =
            render::load_program(display, "sprite_shadow.vert", "sprite_shadow.frag").unwrap();
    ...
    pub fn reload_shaders<F: Facade>(&mut self, display: &F) {
        ...
        match render::load_program(display, "sprite_shadow.vert", "sprite_shadow.frag") {
            Ok(program) => self.shadow_program = program,
            Err(e) => println!("Shader error: {:?}", e),
        }
    }
...
fn make_sprites(world: &World, viewport: &Viewport) -> Vec<(DrawSprite, (u32, u32))> {
    ...
    {
        let mut push_sprite = |pos: Point, kind: &str| {
            let sprite = DrawSprite { kind: kind.to_string() };
        ...
        for entity in seen.iter() {
            if let Some(pos) = world.position(*entity) {
                ...
                if let Some(appearance) = world.ecs().appearances.get(*entity) {
                    push_sprite(pos, &appearance.kind);
                }
            }
        }
