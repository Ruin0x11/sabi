...
fn member_name(uuid: &Uuid, status: &PartyMemberStatus, world: &World) -> String {
    match *status {
        PartyMemberStatus::Active => {
            world.entity_by_uuid(*uuid).map(|e| e.name(world)).unwrap_or("(unknown)".to_string())
        },
        PartyMemberStatus::Waiting(ref loadout) |
        PartyMemberStatus::Dead(ref loadout) => {
            loadout.names.clone().map(|n| n.name.clone()).unwrap_or("(unknown)".to_string())
        }
    }
}
...
impl Party {
    ...
    pub fn get_list(&self, world: &World) -> Vec<(Uuid, String)> {
        self.members.iter().map(|(uuid, status)| (*uuid, member_name(uuid, status, world))).collect()
    }
